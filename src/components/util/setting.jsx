...
//setup interceptor (midlelewware) cho tất cả request (thông tin gửi đi đến server) và respone (kết quả nhận từ server)
...
http.interceptors.request.use((req)=>{
    req.headers={
        ...req.headers, // giữ lại cac api có header riêng
...
http.interceptors.response.use((res)=>{
    ...
    const jwtdecodeJWT = decodeJWT(localStorage.getItem(TOKEN))
    if(jwtdecodeJWT){//Khi decode token thành công mới check token còn hay hết hạn về refesh token
        const isExpired = isTokenExpired(localStorage.getItem(TOKEN));
        console.log('isExpired',isExpired)
        //Nếu trước khi gửi token về server mà token đó hết jamk thì gọi api refesh token
    if(isExpired){
        try{
    ...
    //Nếu thành công thì lưu lại token
    ...
    const isExpired = isTokenExpired(localStroage.getItem(TOKEN))
    ...
    switch(err?.response.status){
        ...
        case 401:{
            ...
            //         //Refesh token
            //         try{
            //         const result = await http.get('https://apistore.cybersoft.edu.vn/api/Users/RefeshToken');
        ...
        case 403:{
            ...
            alert('Yêu cầu quản trị viên mới có quyền truy cập vào được !')
            navigateHistory.pusk('/')
        };break;
